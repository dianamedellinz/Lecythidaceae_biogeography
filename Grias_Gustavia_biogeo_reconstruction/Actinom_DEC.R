...
setwd("/Users/dianamedellin/Documents/Lecythidaceae/Biogeography/Biogeo_analyses_2025/Northern_Andes/Grias_Gustavia_DEC")
...
trfn =  "Grias_Gustavia.nwk"
moref(trfn)
tr = read.tree(trfn)
plot(tr,no.margin=TRUE)
head(tr)

# Checking on the tree
plot(tr)
axisPhylo()

#-------------------
# Loading and checking geographic file
geogfn = "Grias_Gustavia_areas.txt"
...
#-------------------
# Checking the maximum range size in my geog_data
max(rowSums(dfnums_to_numeric(tipranges@df)))
#4

# Setting the maximum range size based on my geog_data
max_range_size = 4
# A value of 7 means that even though none of my modern taxa occur in more than one area,
...
# Checking the number of states which will influence the total analysis time (if more than 500-600 calculations will get really slow)
numstates_from_numareas(numareas=7, maxareas=7, include_null_range=TRUE)

#######################################################
# Run DEC (preparing objects for DEC run)
######################################################
...
BioGeoBEARS_run_object$num_cores_to_use = 4 # this might  change depending on the computer
...
#-------------------
resfn = "Grias_Gustavia_DEC.Rdata" # Name file in which to save the run
...
# 1. Define area names
areanames <- c("I", "C", "A", "V", "M", "W", "J")

# 2. Define list of colors
mypalette <- c("red", "blue", "orange", "turquoise", "yellow", "green", "#FF00FF")
...
pdffn = "Grias_Gustavia_DEC_reconstruction.pdf"
pdf(pdffn, width=19, height=20)

# Plot ancestral states - DEC
analysis_titletxt ="BioGeoBEARS DEC on Grias+Gustavia"
...
# States
plot_BioGeoBEARS_results(results_object, colors_list_for_states = my_colors_list, analysis_titletxt, plotwhat="text", label.offset=0.8, tipcex=1.3, statecex=0.9, splitcex=0.8, titlecex=0.8, plotsplits=TRUE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)

# Pie chart
plot_BioGeoBEARS_results(results_object, colors_list_for_states = my_colors_list, analysis_titletxt, plotwhat="pie", label.offset=0.8, tipcex=1.3, statecex=0.6, splitcex=0.9, titlecex=0.8, plotsplits=FALSE, cornercoords_loc=scriptdir, include_null_range=TRUE, tr=tr, tipranges=tipranges)
